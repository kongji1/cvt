...
- `/`：CPA / sub2api 账号凭证转换
- `/session/`：ChatGPT JSON / Session 转换，可输出 CPA、sub2api、Codex、9router、Cockpit、AxonHub 和 Codex-Manager 格式

两个页面均不上传输入内容；ChatGPT Session 转换器由原 `json.caoo.kdns.fr` 项目合并而来，旧站暂时保留作为兼容入口。
